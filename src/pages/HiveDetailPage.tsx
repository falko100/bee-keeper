...
export default function HiveDetailPage() {
  ...
  const isOwner = hive.userId === user?.id;
  ...
  return (
    <div>
        ...
        <div className="flex flex-wrap items-start justify-between gap-4">
          <div>
            ...
            <div className="flex flex-wrap items-center gap-3 mt-2 text-sm text-gray-500">
              ...
              {!isOwner && <Badge className="bg-blue-50 text-blue-700">Shared with you</Badge>}
            </div>
            {hive.notes && <p className="text-sm text-gray-600 mt-3">{hive.notes}</p>}
          </div>
          {isOwner && (
            <div className="flex items-center gap-2">
              <button onClick={() => setShowShare(true)} className="p-2 rounded-lg hover:bg-gray-100 text-gray-500 hover:text-gray-700" title="Share">
                <Share2 size={18} />
              </button>
      ...
      <ConfirmDialog
        ...
        onConfirm={async () => { await deleteHive(hive.id); navigate('/hives'); }}
        title="Delete Hive"
      ...
      <ConfirmDialog
        ...
        onConfirm={async () => { if (deleteInspId) await deleteInspection(deleteInspId); }}
